...
class SimpleXMLElement extends \Phi\DOMTemplate\XML
{
    ...
    public function html($content = null)
    {
        ...
        $node = new SimpleXMLElement('<?xml version="1.0"?><innerHTML>' . $content . '</innerHTML>');
    ...
    {
        if (is_string($node)) {
            $node = new SimpleXMLElement('<?xml version="1.0"?><innerHTML>' . $node . '</innerHTML>');
        }
